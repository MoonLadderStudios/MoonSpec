---
description: Implement the single-story Moon Spec task breakdown with TDD and final verification.
handoffs:
...
3. Load and analyze the implementation context:
   ...
   - **IF EXISTS**: Read `specs/breakdown.md` for source design coverage and cross-spec dependency context when this spec was generated by `/speckit.breakdown`
   - Confirm `spec.md` contains exactly one independently testable user story. If it contains multiple stories, STOP and tell the user to split the design with `/speckit.breakdown` or regenerate a one-story spec with `/speckit.specify`.
...
5. Parse tasks.md structure and extract:
   ...
   - **Execution flow**: TDD order, dependency requirements, and final `/speckit.verify` task
